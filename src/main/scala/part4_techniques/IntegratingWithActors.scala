...
object IntegratingWithActors extends App {
  ...
  val materializedActorRef = actorPoweredSource.to(
    Sink.foreach[Int](number => println(s"actor powered flow number: $number"))
  ).run()
  ...
  class DestinationActor extends Actor with ActorLogging {
    override def receive: Receive = {
      ...
      case StreamComplete =>
        ...
        context.stop(self)
      case StreamFail(ex: Throwable) =>
        log.warning(s"stream failed: $ex")
      case message =>
        log.info(s"message $message")
        sender() ! StreamAck // needed else backpressure
    }
  }
